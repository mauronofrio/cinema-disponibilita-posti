...
void main() {
  group('parseUciProgrammingDay', () {
    ...
    setUpAll(() {
      final raw = File('test/fixtures/uci_programming_sample.json').readAsStringSync();
      filmsJson = (json.decode(raw) as Map<String, dynamic>)['data'] as List<dynamic>;
    });

    test('flattens every format/variant performance into one session list per film', () {
    ...
    test('carries room_id through as webticScreenId, matching a known real session', () {
      final date = DateTime(2026, 7, 7);
      final days = parseUciProgrammingDay(filmsJson, date);
      final minions = days.firstWhere((d) => d.title == 'Minions & Monsters');
      // Confirmed against a real booking: session 83114 is Sala 5, room_id 104.
      final session = minions.sessions.firstWhere((s) => s.sessionId == '83114');
    ...
    test('sessions within a film are sorted by start time', () {
      ...
      for (var i = 1; i < minions.sessions.length; i++) {
        expect(minions.sessions[i].startTime.isAfter(minions.sessions[i - 1].startTime), isTrue);
      }
    });
  ...
  group('parseUciSeatMap', () {
    ...
    setUpAll(() {
      payload = UciSeatMapPayload(
        screenResponseBody: File('test/fixtures/uci_screen_sample.json').readAsStringSync(),
        occupancyResponseBody: File('test/fixtures/uci_occupancy_sample.json').readAsStringSync(),
      );
    });

    test('builds a row per distinct Row value, sized to the widest column seen', () {
    ...
    test('a seat listed in Occupancy is occupied, matching the captured example', () {
      final seatMap = parseUciSeatMap(payload);
      final seat = seatMap.rows.expand((r) => r.seats).whereType<Seat>().firstWhere((s) => s.name == 'H/11');
    ...
    test('a seat absent from Occupancy is available, not just "unknown"', () {
      ...
      final seat = seatMap.rows.expand((r) => r.seats).whereType<Seat>().firstWhere((s) => s.name == 'C/1');
      expect(seat.status, SeatStatus.available);
    });

    test('DISABILE seats read as accessibility when not occupied', () {
      final seatMap = parseUciSeatMap(payload);
      final seat = seatMap.rows.expand((r) => r.seats).whereType<Seat>().firstWhere((s) => s.name == 'P.D.');
